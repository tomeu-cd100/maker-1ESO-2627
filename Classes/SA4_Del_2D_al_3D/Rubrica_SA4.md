# Rúbrica SA4 — «Del 2D al 3D»

Nivells: **NA / AS / AN / AE**. Vegeu `Avaluació/Rubrica_general_competencial.md`.

| Criteri | NA | AS | AN | AE |
|---------|----|----|----|----|
| **Modelatge 3D (CA2.2)** | Encara no modela | Combina cossos amb ajuda | Combina ≥3 cossos i ≥1 forat | Model elaborat amb diverses operacions i autonomia |
| **Geometria, mesura i escala (CA2.3)** | Mides encara irreals | Mides aproximades | Mides correctes i gruix adequat | Mides precises i optimitzades per imprimir |
| **Exportació i comprensió del flux (CA2.2)** | Encara no exporta | Exporta amb ajuda | Exporta STL correctament | Exporta i comprova/optimitza el model |
| **Reflexió (CA1.4)** | Encara no reflexiona | Reflexió breu | Identifica aprenentatges | Reflexió rica sobre el pas 2D→3D |
| **Documentació (CA5.2)** | Encara no documenta | Incompleta | Captures i procés | Completa i reflexiva |

**Observacions:**
