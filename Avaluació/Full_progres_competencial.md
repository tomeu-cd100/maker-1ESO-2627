# Full de progrés competencial individual — «Aula Maker» (1r ESO)

> **Per què existeix:** la qualificació és una valoració **global i evolutiva** que prioritza
> «l'evolució i les darreres evidències de cada competència» (`Criteris_i_qualificacio.md`).
> Aquest full és **l'instrument que fa possible aquesta lectura**: sense ell, al juny
> l'evolució es reconstrueix de memòria — i la memòria afavoreix els alumnes salients (els
> molt bons i els molt difícils), no els invisibles del mig. **Una pàgina per alumne/a**
> (20 pàgines per grup); s'omple en 2-3 minuts en tancar cada SA, a partir de la rúbrica o del
> checklist de criteris d'èxit.
>
> 💻 **Versió recomanada: digital** — la pestanya `Progrés` del quadern digital
> (`Quadern_digital_docent.md`), amb format condicional que fa visible la trajectòria d'un cop
> d'ull. Aquest document en paper queda com a alternativa i com a especificació del contingut.

---

## Full individual (imprimir una còpia per alumne/a)

**Nom i cognoms:** ______________________________  **Grup (A/B):** ______

Anoteu el nivell (**NA / AS / AN / AE**) en tancar cada SA, **només** a les competències que
la SA treballa (vegeu la matriu de `Criteris_i_qualificacio.md` §4). Cel·la buida = no s'hi
ha treballat.

| Competència | SA1 | SA2 | SA3 ⭐ | SA4 | SA5 | SA6 ⭐ | SA7 | SA8 | SA9 ⭐ | **Global curs** |
|-------------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| **CE1** Procés i disseny | | | | | | | | | | |
| **CE2** Disseny 2D/3D | | | | | | | | | | |
| **CE3** Fabricació | | | | | | | | | | |
| **CE4** Immersiu | | | | | | | | | | |
| **CE5** Equip, doc., comunicació | | | | | | | | | | |
| **CE6** Seguretat, sost., ètica | | | | | | | | | | |
| **TE** Participació equitativa | | | | | | | | | | |
| **TA** Orientació a l'usuari | | | | | | | | | | |

**Valoracions trimestrals** (judici global del trimestre, no mitjana):
T1: ______  T2: ______  T3: ______

**Objectius personals de l'alumne/a** (els que escriu al tancament del diari; anoteu-ne el
seguiment):
- SA__ → SA__: ______________________________ (assolit? __ )
- SA__ → SA__: ______________________________ (assolit? __ )
- SA__ → SA__: ______________________________ (assolit? __ )

**Observacions de curs** (NESE, adaptacions, carnets, fets rellevants):

---

## Com llegir-lo (la clau de tot l'instrument)

- **Cada fila es llegeix d'esquerra a dreta com una trajectòria**, no com una llista de notes:
  un alumne NA-AS-AS-AN en CE2 està **progressant** (i això és el que es qualifica); un
  AN-AN-AS-AS està **retrocedint** i demana una conversa, no una mitjana.
- **La valoració global de cada CE és el nivell que l'alumne demostra al final** (les darreres
  evidències), matisat per la trajectòria — mai la mitjana aritmètica de la fila.
- Les SA amb ⭐ (SA3, SA6, SA9) porten rúbrica completa i pesen més com a evidència; a la
  resta, el nivell surt del **checklist de criteris d'èxit** (vegeu la jerarquia d'instruments,
  `Criteris_i_qualificacio.md` §2·bis). Aquest full no és feina nova: és **on conflueix** el
  que ja recolliu.

## Rutines d'ús

- **En tancar cada SA** (els 2-3 min de passar la rúbrica/checklist): ompliu la columna.
- **En codocència:** cada docent omple les files que avalua (vegeu el repartiment a
  `Programació didàctica/Codocencia_desdoblament.md` §6) — el full és el punt de trobada dels
  dos fulls de seguiment.
- **Amb l'alumne/a:** ensenyeu-li el seu full **una vegada per trimestre** (o quan demani "quina
  nota tinc"): veure la pròpia fila de CE2 passar de NA a AN és el retorn més motivador que
  existeix, i fa comprensible la qualificació competencial a les famílies.
- **Al juny:** la columna «Global curs» s'omple fila a fila mirant la trajectòria. La nota final
  és el judici global de les **6 competències específiques (CE1–CE6)**; les files **TE** i **TA**
  són **informatives** i **maticen** CE5/CE6 i CE1 (no són competències noves ni se sumen a part,
  per no comptar dues vegades el mateix).

> Relacionat amb: `Criteris_i_qualificacio.md` (§2·bis i §4), les `Rubrica_SAx.md`,
> `Diari_de_taller.md` (objectius personals) i `Avaluacio_inicial.md` (punt de partida).
